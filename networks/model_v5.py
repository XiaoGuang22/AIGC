...
class CrossModalDetector(nn.Module):
    def __init__(self,
        ...
        try:
            ...
            print("Local path failed, downloading openai/clip-vit-large-patch14...")
            self.clip_model = CLIPModel.from_pretrained("openai/clip-vit-large-patch14")

        # 拆解 CLIP，拿到我们需要的组件
        self.vision_encoder = self.clip_model.vision_model  # 输出 1024
        self.visual_proj = self.clip_model.visual_projection  # Linear(1024 -> 768)
    ...
    def forward(self, images, input_ids, attention_mask):
        ...
        #     need_weights = True       # 代码加了这段，和attn_weights
        # )
        ...
        feat_text = text_features[:, 0, :]

        # 3. 全量拼接
        # 维度变成 [B, 768 * 4] = [B, 3072]
        #combined_features = torch.cat([V_ln, A_ln, diff, mult], dim=-1)
        combined_features = torch.cat([feat_cls_768, feat_text], dim=-1)

        # 传入带有 GELU 的非线性分类器
        logits = self.classifier(combined_features)
        #return logits, V_ln, A_ln, attn_weights    # 代码加了attn_weights
        return logits