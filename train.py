...
class ContrastiveConflictLoss(nn.Module):
    def __init__(self):
        super(ContrastiveConflictLoss, self).__init__()
        #self.margin = margin

    def forward(self, v_feat, a_feat, labels):
        # 计算余弦相似度并转为距离
        cos_sim = F.cosine_similarity(v_feat, a_feat, dim=1)
        dist = 1.0 - cos_sim

        # 真实图像损失 (Label=0): 拉近距离
        loss_real = (1 - labels) * (dist ** 2)
        # 生成图像损失 (Label=1): 推开距离
        #loss_fake = labels * F.relu(self.margin - dist) ** 2

        return torch.mean(loss_real)
...
CONFIG = {
    ...
    "LAMBDA_WEIGHT": 0.5,   # 对比学习损失权重
    "NUM_CLASS": 2,


    "SAVE_FREQ": 5,
    "SAVE_DIR": "checkpoints/test39",
    "LOG_DIR": "results/logs",
    "JSON_PATH": "data/dataset_sdv4_inblip.json",
    # "JSON_PATH": "data/test_benchmarks_inblip2/test_stable_diffusion_v_1_4_inblip.json",
    "VAL_UNSEEN_JSON": "data/test_benchmarks_inblip2/test_Midjourney_inblip.json",
...
def validate(model, loader, criterion, device, desc="Validation"):
    ...
    with torch.no_grad():
        for images, input_ids, attention_mask, labels in loop:
            ...
            #outputs, _, _ = model(images, input_ids, attention_mask)
...
def train():
    ...
    if tokenizer_type == "clip":
        ...
        print(f" Using {tokenizer_type.upper()} Tokenizer from: {current_tokenizer_path}")

    # 2. 加载数据
    log_print(">>> Initializing Datasets...")
    # A. 训练集 - 保持 Blur/JPEG 增强
    ...
    #criterion_conflict = ContrastiveConflictLoss().to(DEVICE)
    ...
    for epoch in range(CONFIG["NUM_EPOCHS"]):
        ...
        for batch_idx, (images, input_ids, attention_mask, labels) in enumerate(loop):
            ...
            with autocast():
                # 1. 解包 3 个返回值
                #outputs, v_feat, a_feat = model(images, input_ids, attention_mask)
                outputs = model(images, input_ids, attention_mask)
                # 2. 分别计算两个 Loss
                batch_loss_cls = criterion_cls(outputs, labels)
                #batch_loss_conflict = criterion_conflict(v_feat, a_feat, labels)
                # 3. 融合总 Loss
                loss = batch_loss_cls # + lambda_weight * batch_loss_conflict
            ...
            # train_loss_conflict += batch_loss_conflict.item()
            ...
            loop.set_postfix(
                loss=loss.item(),
                cls_loss=batch_loss_cls.item(),
                # conf_loss=batch_loss_conflict.item(),
        ...
        # 计算细分 Loss 的平均值
        avg_train_cls = train_loss_cls / len(train_loader)
        # avg_train_conf = train_loss_conflict / len(train_loader)

        train_acc = 100 * correct / total

        # === 验证 ===
        loss_seen, acc_seen = validate(model, val_seen_loader, criterion_cls, DEVICE, desc="Val Seen")

        if has_unseen:
            loss_unseen, acc_unseen = validate(model, val_unseen_loader, criterion_cls, DEVICE, desc="Val Unseen")
        ...
        log_msg = (f"Epoch [{epoch + 1}/{CONFIG['NUM_EPOCHS']}] "
                   f"LR: {current_lr:.6f} | "
                   # f"Loss: {avg_train_loss:.4f} (Cls:{avg_train_cls:.4f}, Conf:{avg_train_conf:.4f}) | "
                   f"Loss: {avg_train_loss:.4f} (Cls:{avg_train_cls:.4f}) | "
